...
def process_cosponsor(chamber, session):
    ...
    np.save('processed_data/happy_wcg_%s_%s_party' % (chamber, session), member_to_party)
...
def process_voting_data(chamber, session):
    ...
    bill_filenames = []
    for year in os.listdir('raw_%s_vote_data/data/%s/votes' % (chamber, session)):
        for bill in os.listdir('raw_%s_vote_data/data/%s/votes/%s' % (chamber, session, year)):
            bill_filenames.append('raw_%s_vote_data/data/%s/votes/%s/%s/data.json' % (chamber, session, year, bill))
    ...
    print 'valid bills: %s' % len(bill_filenames)
    total_bills = len(bill_to_id)
    return member_to_party, member_to_bills, total_bills
...
def process_voting(chamber, session):
    ...
    np.save('processed_data/happy_wvg_%s_%s_weights_total' % (chamber, session), weights_total)
    np.save('processed_data/happy_wvg_%s_%s_weights_by_i' % (chamber, session), weights_by_i)
    np.save('processed_data/happy_wvg_%s_%s_party' % (chamber, session), member_to_party)
